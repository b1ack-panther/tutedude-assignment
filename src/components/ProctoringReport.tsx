...
export const ProctoringReport: React.FC<ProctoringReportProps> = ({
	...
	const downloadReport = () => {
		...
		// Summary section as key,value rows
		...
		const sessionFields: Record<string, any> = {
			...
			durationMinutes: reportData.duration,
			startTime: new Date(reportData.startTime).toISOString(),
			endTime: reportData.endTime
				? new Date(reportData.endTime).toISOString()
				: "",
			totalEvents: reportData.totalEvents,
			focusLostCount: reportData.focusLostCount,
			suspiciousEventCount: reportData.suspiciousEventCount,
			totalFocusLostSeconds: reportData.summary?.totalFocusLostTime ?? "",
			avgFocusLostSeconds: reportData.summary?.averageFocusLostDuration ?? "",
		};
		Object.entries(sessionFields).forEach(([k, v]) => {
			lines.push(["Summary", k, escapeCsv(v)].join(","));
		});
		...
		// Events table
		lines.push("Events,id,type,severity,description,timestamp,durationSeconds");
		...
		a.href = url;
		a.download = `proctoring-report-${reportData.sessionId}.csv`;
	...
	return (
		<div className="space-y-6 max-w-5xl mx-auto">
			...
			<Card className="p-6 bg-gradient-to-r from-primary/5 to-primary-glow/5 border-primary/20">
				<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-muted-foreground">
									Session Analysis & Integrity Assessment
								</p>
							</div>
						</div>
					</div>
					<div className="text-right space-y-2">
						...
						<Badge
							variant={
								reportData.integrityScore >= 70 ? "default" : "destructive"
							}
							className="text-sm"
						>
				...
				<Card className="p-6">
					...
					<div className="space-y-3">
						...
						<div className="flex justify-between">
							...
							<span className="font-medium">
								{reportData.suspiciousEventCount}
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">Total Focus Lost:</span>
							<span className="font-medium">
								{reportData.summary.totalFocusLostTime}s
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">Avg. Focus Lost:</span>
							<span className="font-medium">
								{reportData.summary.averageFocusLostDuration}s
							</span>
						</div>
					</div>
				</Card>
			...
			{reportData.recordingUrl && (
				<Card className="p-6">
					<h3 className="text-lg font-semibold mb-4">Session Recording</h3>
					<video
						src={reportData.recordingUrl}
						controls
